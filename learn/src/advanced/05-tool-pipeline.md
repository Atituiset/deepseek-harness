...
- 知道 Code Mode 是什么、`run_code` 如何复用这条管线。
...
**1. `tools/pre-execute`（waterfall，签名在 `packages/core/tools/src/index.ts:152`）。** 决策门。监听器返回三种决策（`PreToolDecision`，:588-591）：
...
**2. 单调守卫（monotonic guards）。** `guard()` 注册在 :1110，语义在 JSDoc（:1101-1109）：守卫在 pre-execute 之后评估，任何匹配的守卫可以返回拒绝理由，但**没有守卫能 force-allow 一个被别的守卫拒绝的调用**——「单调」即拒绝只能增加不能撤销。注册路径决定作用范围：普通 context 上的守卫是全局的，`agent.ctx` 上的只作用于那个 agent。所有者级的、不允许被重排的硬策略（而非可协商的审批）应该做成守卫而不是 waterfall 监听器。

**3. `tools/execute`（around waterfall，:163）。** 包裹真正的工具体。这是「around」语义：监听器拿到 `next()`，可以在前后包任何东西——超时、重试、指标。`packages/guard/timeout-policy` 的超时就是这里的一个 wrapper。`ToolDispatchExecution`（:391）给了 wrapper 一个可变视图：允许替换并恢复 `exec.signal` 来施加 deadline，但不能移除它。

**4. `tools/post-execute`（waterfall，:175）。** 拿到已执行的结果，决策三种（`PostToolDecision`，:597-600）：`accept`（可替换 content 或 value 之一，不可同时）、`block`（把结果换成对模型的反馈）、以及两者都可携带的 `additionalContexts`（追加进后续请求的上下文）。结果重写、结果拦截、给模型附加上下文都发生在这。

**5. `finalizeContent` + `tools/result`。** 工具自己声明的 `finalizeContent` 是最后一道只作用于 content 的不变量；之后 `tools/result`（:197）是**只读**通知——此时结果已冻结、已落盘定稿，监听器只能观察（UI 更新、遥测）。
...
- `tools/pre-execute`（:152）：`(exec, next) => Promise<PreToolDecision>`
- `tools/execute`（:163）：`(exec, next) => Promise<ToolExecutionResult>`
- `tools/post-execute`（:175）：`(exec, result, next) => Promise<PostToolDecision>`
- `tools/result`（:197）：`(exec, result) => undefined`——返回 `undefined`，从类型上就断了你改结果的念头。
...
`ToolRuntime`（`packages/core/tools/src/index.ts:787`）是 `ctx.tools` 的实现。四个常用方法：

- `register(definition)`（:1037）——注册全局或调用方 agent 作用域的工具。校验在入口做完：output 必须声明 `{ schema, render, presentationMeta? }`（:1040-1044）、schema 必须是受支持的 JSON Schema（:1045）、`timeoutMs` 必须是正有限数（:1046-1050）、`run_code` 是保留名（:1054）。返回 disposer——注册是 effect，卸载即注销。作用域工具 shadow 同名全局工具（就近优先）。
- `guard(guard)`（:1110）——注册单调守卫，见上。
- `get(name, scope?)`（:1204）——按某个作用域的视角查工具：被 restrict 掉的全局工具读起来就像不存在。
- `schemas(scope?)`（:1234）——把可见工具投影成模型可见 schema。注意「投影」二字：`schemas()` 只放行 name/description/parameters 等白名单字段，execute、presentation 这些回调永远不进模型请求（:1228-1235 注释）。超时元数据也绝不模型可见（:251 附近）。
...
作用域与限制（`restrict`，:1071）的组合规则：restriction 以**交集**组合，过滤的是继承来的全局工具集；作用域自己的注册在过滤之后合并，不受自己的 restriction 影响（:1176-1183）——这保证了子代理的报告工具不会被父代理给它设的能力过滤器剥掉（:1138-1148 的注释讲了这个 bug 史）。
...
- `ToolDefinition.timeoutMs`（:246-250）只是**声明**：注册表校验它是正有限数，但注册表自己不执行超时。执行者是 `@deepseek-ai/dsh-tool-call-timeout-policy`——一个 `tools/execute` 的 around wrapper（`packages/guard/timeout-policy`）。声明与执行分离，所以超时策略是可替换的插件。
...
- `presentationMeta(args, value): JsonValue`（:218）——结果落盘时投影出的**纯函数**元数据，存进 `tool/result` 事件的 `meta` 字段，回放时 UI 靠它重建卡片。
...
工具的并发模式（`ToolExecutionMode`，:344）分 `parallel` 与独占两类，调度器（第 9 章 `tool-calls.ts`）在每次启动前重新查询 `ctx.tools.executionMode(exec)`——所以一个工具可以在注册表里声明「我副作用大，必须独占执行」，调度器会让它成为屏障：前面的并行池排空后它才启动，它跑完后后面的才能动。模式是按调用现场重新分类的，注册表运行期变更会影响尚未启动的调用。

## Code Mode：一条管线，两种呈现

`ToolPresentationMode`（:651）有三个值：`native`（默认，每个可见工具一个 schema）、`code`（模型只看到一个 `run_code` 传输工具 + 生成的 SDK 提示词节，工具调用变成写代码）、`both`。

关键设计：`run_code` 不是一个旁路执行器——它的子调用带着 parent token **重进正常执行管线**（pre-execute、守卫、execute、post-execute 一级不少），所以权限策略对两种呈现一视同仁。注册表侧的配合：任何 agent 都可能给自己选 code mode，所以 `run_code` 名字被无条件保留（:1051-1056）；code 模式下模型直接调用只能命中 `run_code`，嵌套子调用才能命中任意可见工具（`resolveExecution`，:1221）。`code` 模式需要一个 `ctx.codeRuntime` 实现（如 worker-thread provider），缺席时在装配/执行边界报可操作的配置错误（`requireCodeRuntime`，:1019-1029）。

子调用与顶层调用的差异也被仔细设计过（`docs/tool-execution-pipeline.md:60`）：子调用带 parent token（`ToolExecution.parent`），日志里记 `tool/code-dispatch`，拒绝以「绑定拒绝」的形式返回给程序而不是吞掉，并且省略 `additionalContexts` 以保持 call/result 相邻。`Config`（:654）里的 `mode` 是部署默认值，agent preset 可以按 agent 覆盖；`run_code` 程序内重叠子调用的并发上限（:667 的 Config 字段）同样在注册表边界解析（:774）。
...
- `packages/core/tools/README.md` — Code Mode 与作用域注册的细节。
