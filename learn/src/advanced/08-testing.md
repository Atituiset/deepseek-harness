...
| 本地期望输出 | `pnpm run test:expected` | 免 key 的 CLI/进程装配期望，不经录制会话往返 |
| 快照 | `pnpm run test:snapshot` | 免 key 回放录制的 session transcript（headless/SDK/ACP/Web） |
...
**本地期望输出（`pnpm run test:expected`）。** 免 key、不经录制会话往返的装配期望：驱动脚本 `*.expected.e2e.ts` 住在各 app 的 `tests/expected/` 旁，跑构建产物断言 CLI/进程输出。包与脚本的期望走 `test`，浏览器的走 `test:web`。

**真实 API e2e（`pnpm run test:e2e`）。** 带 key 打真实 provider。每个套件在没有自己的 key 时自动 skip（`DEEPSEEK_API_KEY`、`EXA_API_KEY` 等各自独立），所以无 key 的 CI 保持绿。政策原文值得记住：「inference is cheap here」——无 key 测试只证明管道通，只有带 key 的运行证明 agent 对真实模型工作。最有价值的是 smoke：启动真实 profile、发一个 prompt、检查世界。

**快照（`pnpm run test:snapshot`）。** 免 key 的外部行为门禁。每个顶层场景有一份录制的 `session.jsonl`：它既供用户输入，又供模型回放，回放后同时充当期望的持久化结果。进程场景都经真实 `dsh` 启动——headless 钉一次性行为、SDK 钉持续控制、ACP 钉自动化协议、Web 在同一份 session 旁保留浏览器/ARIA 证据。`snapshot.yml` 声明 profile、组合/头类、录制策略与 workspace 事实；typed token 保住父/子身份关系，只有 header 钉住提示词/schema 旁路文件。三个动词：
...
**Web 浏览器快照（`pnpm run test:web`）。** Chromium 回放会话驱动的输出（`snapshots/web/`）与 UI-only 输出（`apps/web/tests/expected/`）。CI 强制只读 `DSH_SNAPSHOT=replay`，从不写期望输出；录制和 refresh 只在本地做。它先跑 build（插件 CSS 需要构建产物），所以它同时也是一条「构建没坏」的信号。

## 快照的组织方式

场景集中归属（`docs/testing.md:52` 起）：headless/SDK/ACP/Web 的录制都在 `snapshots/session/`、`snapshots/sdk/`、`snapshots/acp/`、`snapshots/web/` 四个集中目录下，`dsh-session-snapshot` 套件工厂拥有共享的存储规则与 profile 适配器；不是由录制 session 驱动的期望输出，留在各自 app/package/script 的 `tests/expected/` 下且不用 `*.snapshot.ts` 后缀。Web 渲染可以显式借用另一个场景的 canonical session。快照 diff 的审查规则不变：逐行 review，`workspace.expected/` 树只被 record 比对、永不被 record/refresh 重写。
...
**5. 守卫要真的守得住。** 对没有 `inject` 的组合类插件，一个 Loader smoke 在「默认导出顶替了必需具名导出」时照样是绿的——所以要显式断言 `expect('default' in mod).toBe(false)` 并做 `unwrapExports` 往返，而且要**证明**它：引入回归、看它变红、再还原（`docs/testing.md:39`）。一条从没红过的守卫不是守卫。
...
五问都能在动手前回答——`docs/testing.md` 的「When a snapshot test is required」一节要求「计划阶段点名每一层覆盖，并在实现前验证 harness 能表达它」。
...
一个反直觉但重要的纪律：如果你怀疑某个测试其实守不住它声称的东西（比如换了实现它也不红），**先引入回归看它变红再还原**——这是仓库对「守卫有效性」的标准验证法（`docs/testing.md:39`）。
...
想亲眼看「行为变更如何体现为快照 diff」：改一句 headless persona（`packages/bundle/headless/cordis.patch.yml` 的 `persona`），`pnpm run test:snapshot:refresh`，然后 `git diff` 看哪些 fixture 动了——这就是评审者审查一次模型可见改动时看到的东西。
...
`docs/testing.md:52` 起的规则没有例外口：**每个非平凡的模型可见、协议可见或人类可见的改动，都要在同一个 PR 里新增或更新一个免 key 的录制会话场景。** 包测试、e2e 断言、mock-only 组合、PR 论述都不能替代装配后的 transcript；harness 表达不了你的场景时，在同一个改动里扩展 harness。规划能力接缝、生命周期变体或 transcript 表面时，要在计划阶段就点名每一层覆盖，并在实现前验证 harness 能表达它。

这条规则的直觉解释：单测证明零件对，快照证明整机说人话。Agent 产品的回归大多数是「零件全绿、整机说胡话」，只有装配后的 transcript 抓得住。

## 动手练习

**练习 1：跑快照全量。** 先 `pnpm install && pnpm run build`，然后 `pnpm run test:snapshot`——不需要任何 API key。观察输出里 ACP 与 headless 场景的列表，挑一个名字用 `pnpm run test:snapshot -- -t <名字>` 单跑，然后打开它对应的场景目录（`snapshots/session/`、`snapshots/acp/`、`snapshots/sdk/`、`snapshots/web/` 之一）里的 `session.jsonl` 与 `snapshot.yml`，对照第 10 章的事件词汇读懂录制的 JSONL。
